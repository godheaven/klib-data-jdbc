...
public class JdbcCache {
    ...
    public static SqlBase sqlBase(Class<? extends Mapping> clazz, boolean loadAll) {
        ...
        if (sqlBase == null) {
            ...
            if (view != null) {
                ...
                sqlBase =
                        new SqlBase(creator.getTable(), creator.generate(), creator.getAliasMap());
                sqlBaseCache.put(key, sqlBase);
            }
        }
        return sqlBase;
                    ...
                    new AbstractRowMapper() {
                        ...
                        public Object mapRow(ResultSet rs, int i) throws SQLException {
                            ...
                            try {
                                ...
                                for (Field field : clazz.getDeclaredFields()) {
                                    ...
                                    if (column != null) {
                                        ...
                                        if (column.parser() != null
                                                && column.parser() == EnumParser.class) {
                                            ...
                                            field.set(
                                                    ...
                                                    EnumParser.parse(
                                                            column.parserResult(),
                                                            isNumeric(enumId)
                                                                    ? Integer.valueOf(enumId)
                                                                    : enumId));
                                        } else if (column.parser() != null
                                                && column.parser() == JsonListParser.class) {
                                            String json = rs.getString(column.name());
                                            field.set(
                                                    object,
                                                    JsonListParser.parse(
                                                            column.parserResult(), json));
                                        } else if (column.parser() != null
                                                && column.parser() == JsonParser.class) {
                                            String json = rs.getString(column.name());
                                            field.set(
                                                    object,
                                                    JsonParser.parse(column.parserResult(), json));
                                        } else if (column.parser() != null
                                            ...
                                            String json =
                                                    new String(
                                                            (byte[]) bytes,
                                                            DEFAULT_CHARSET); // TODO: parametrizar
                                            // el encoding
                                            field.set(
                                                    object,
                                                    ByteaJsonListParser.parse(
                                                            column.parserResult(), json));
                                        } else if (column.parser() != null
                                                && column.parser() == ByteaJsonParser.class) {
                                            byte[] bytes = rs.getBytes(column.name());
                                            String json =
                                                    new String(
                                                            (byte[]) bytes,
                                            ...
                                            field.set(
                                                    object,
                                                    ByteaJsonParser.parse(
                                                            column.parserResult(), json));
                                        } else {
                                            Object columnObject =
                                                    rs.getObject(
                                                            !Utils.isNullOrEmpty(column.alias())
                                                                    ? column.alias()
                                                                    : column.name());
                                            if (columnObject instanceof BigDecimal
                                                    && field.getType() == Long.class) {
                                                field.set(
                                                        object,
                                                        (((BigDecimal) columnObject).longValue()));
                                            } else if (columnObject instanceof Integer
                                                    && field.getType() == Long.class) {
                                                field.set(
                                                        object,
                                                        (((Integer) columnObject).longValue()));
                                            } else if (columnObject instanceof byte[]
                                                    && field.getType() == StringWriter.class) {
                                                ...
                                                field.set(
                                                        object,
                                                        str); // TODO: parametrizar el encoding
                                            } else if (columnObject instanceof java.sql.Date
                                                    && field.getType() == LocalDate.class) {
                                                field.set(
                                                        object,
                                                        ((java.sql.Date) columnObject)
                                                                .toLocalDate());
                                            } else if (columnObject instanceof java.sql.Timestamp
                                                    && field.getType() == LocalDateTime.class) {
                                                field.set(
                                                        object,
                                                        ((java.sql.Timestamp) columnObject)
                                                                .toLocalDateTime());
                                            } else if (columnObject instanceof java.sql.Timestamp
                                                    && field.getType() == LocalDate.class) {
                                                field.set(
                                        ...
                                        ColumnGroup columnMapping =
                                                field.getAnnotation(ColumnGroup.class);
                                        if (columnMapping != null) {
                                            try {
                                                ...
                                                field.set(
                                                        object,
                                                        rowMapper(columnMapping.result(), loadAll)
                                                                .mapRow(rs, i));

                                            } catch (Exception ex) {
                                                // Internal mapping is not feasible or SQL
                                                // incomplete
                                                field.setAccessible(true);
                                            ...
                                            if (joinTable != null) {
                                                try {
                                                    ...
                                                    field.set(
                                                            object,
                                                            rowMapper(joinTable.table(), loadAll)
                                                                    .mapRow(rs, i));
                                                } catch (Exception ex) {
                                                    // Internal mapping is not feasible or SQL
                                                    // incomplete
                                                    ...
                                                    // TODO: se debe implementar el lanzar excepcion
                                ...
                                throw new SQLException(
                                        "Error performing automatic rowmapper of "
                                                + clazz
                                                + ": "
                                                + ex.getMessage(),
                                        ex);
    ...
    private static void translationMapExtract(
            Class clazz, Map<String, String> translationMap, String prefix, Table parent) {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (column != null) {
                ...
                if (columnMapping != null) {
                    translationMapExtract(
                            columnMapping.result(),
                            translationMap,
                            prefix + field.getName() + ".",
                            (table != null) ? table : parent);
                } else {
    ...
    private static void prepareCreator(
            Table parent,
            JoinTable joined,
            Class currentClazz,
            boolean loadAll,
    ...
    static class SQLCreator {
        ...
        public void addJoinTable(Table parent, JoinTable joinTable) {
            ...
            sqlJoins.append(" ")
                    .append(joinTable.operator().toString().replace("_", " "))
                    .append(" ")
            ...
            sqlJoins.append(" ON ")
                    .append(aliasParent)
                    .append(".")
                    .append(joinTable.foreignKey())
                    .append("=")
                    .append(alias)
                    .append(".")
                    .append(tableJoin.keys()[0]);
        }
        ...
        private void addColumn(String tableName, String column, String alias) {
            columns.add(
                    getAlias(tableName)
                            .concat(".")
                            .concat(column)
                            .concat(!Utils.isNullOrEmpty(alias) ? " AS ".concat(alias) : ""));
        }
    }
}
